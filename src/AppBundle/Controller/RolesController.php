...
*/     public function editarAction($id){          
                ...
                if($this->getRequest()->isMethod('POST')){             
                    ...
                    $params = array('rol' => $rol);
                       ...
                       return $this->render('Roles/editar.html.twig', $params);              }          /**      * @Route("/roles/eliminar/{id}/")      */     public function eliminarAction($id){                  $repository = $this->getDoctrine()->getRepository('AppBundle:Rol');         $rol = $repository->find($id);                  $em = $this->getDoctrine()->getManager();         $em->remove($rol);         $em->flush();         $this->addFlash('notice', 'Registro eliminado satisfactoriamente');         return $this->redirect('../../');              }     
}